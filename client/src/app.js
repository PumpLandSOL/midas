// Midas (3,3) dashboard — metrics, live rebase, stake/bond/claim.
(function () {
  const $ = (id) => document.getElementById(id);
  const sleep = (ms) => new Promise((r) => setTimeout(r, ms));
  let M = null, A = null, wallet = localStorage.getItem('midas_w') || '';
  ...
  function setWallet(a) {
    if (a && isW(a)) { wallet = a; localStorage.setItem('midas_w', a); renderWallet(); loadAccount(); }
    else { wallet = ''; A = null; localStorage.removeItem('midas_w'); renderWallet(); ['yStaked', 'yBalance', 'yNext'].forEach((id) => $(id).textContent = '—'); renderYourBonds(); }
  }
  ...
  function renderMetrics() {
    ...
    $('mBacking').textContent = 'backing $' + M.backingPerToken.toFixed(6) + ' / $MIDAS';
    ...
    if (M.leaderboard && M.leaderboard.length) {
      ...
      $('lbRows').innerHTML = M.leaderboard.map((b, i) => `<div class="row"><span>${i + 1}. <b style="color:var(--ink);font-family:'JetBrains Mono',monospace">${b.wallet}</b></span><span><b class="tl">${tok(b.staked)} sMIDAS</b> <span style="color:var(--mut)">· ${pct(b.share)}</span></span></div>`).join('');
    }
    calc();
  ...
  function renderAccount() {
    if (!A) return;
    $('yBalance').textContent = tok(A.balance) + ' $MIDAS';
    $('yNext').textContent = '+' + (A.staked * M.rate).toFixed(4) + ' $MIDAS';
  }
  ...
  function renderYourBonds() {
    ...
    $('yourBonds').innerHTML = A.bonds.map((b) => `
      <div class="yb"><span>${b.market} · <b style="color:var(--ink)">${tok(b.payout)}</b> $MIDAS</span>
  ...
  $('stakeBtn').onclick = async () => {
    ...
    if (r.error) return toast(r.error); A = r; reanchor(); renderAccount(); $('stakeAmt').value = ''; toast((stakeMode === 'stake' ? 'Staked ' : 'Unstaked ') + tok(amt) + ' $MIDAS (3,3)');
  };
  async function doBond(id) {
    ...
    if (r.error) return toast(r.error); A = r; renderYourBonds(); loadMetrics(); $('bondAmt_' + id).value = ''; toast('Bonded — ' + tok(r.payout) + ' $MIDAS vesting');
  }
  async function doClaim(autostake) {
    const r = await post('/api/claim', { wallet, autostake });
    if (r.error) return toast(r.error); A = r; reanchor(); renderAccount(); renderYourBonds(); toast(autostake ? 'Claimed & staked ' + tok(r.claimed) : 'Claimed ' + tok(r.claimed) + ' $MIDAS');
  }
  ...
  function tick() {
    ...
    $('mStaked').textContent = tok(anchor.totalStaked * ratio) + ' sMIDAS';
    if (A && anchor.agons) { $('yStaked').textContent = (anchor.agons * li.index).toFixed(4) + ' sMIDAS'; }
    else if (A) $('yStaked').textContent = '0.0000 sMIDAS';
  }
  // ---- calculator ----
  function calc() {
    ...
    $('calcOut').textContent = tok(out) + ' $MIDAS';
