// Midas ($MIDAS) — OHM-style (3,3) staking + bonding + treasury protocol.
...
const TOKEN = process.env.TOKEN_TICKER || 'MIDAS';
const DATA_PATH = process.env.DATA_PATH || path.join(ROOT, 'data.json');
const MIDAS_MINT = process.env.MIDAS_MINT || '';                 // CA bar (dormant)
...
const BONDS = [
  ...
  { id: 'lp', name: 'MIDAS-ETH LP', discount: 0.13, vestDays: 5 },
...
function metrics() {
  ...
  return {
    ...
    leaderboard, stakers: leaderboard.length, mint: MIDAS_MINT,
  };
}
...
http.createServer(async (req, res) => {
  ...
  if (u === '/api/config') return json(res, 200, { token: TOKEN, rebaseSec: REBASE_SEC, apy: APY_TARGET, mint: MIDAS_MINT, network: 'robinhood-chain' });
  ...
  serve(req, res);
}).listen(PORT, () => console.log('Midas ($' + TOKEN + ') on :' + PORT + ' — APY ' + APY_TARGET + '%, rebase ' + REBASE_SEC + 's'));
